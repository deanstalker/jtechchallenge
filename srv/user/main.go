...
import (
	...
	db2 "github.com/deanstalker/jtechchallenge/srv/user/db"
...
func main() {
	...
	repo := db2.NewUserRepo(db)

	if err := user.RegisterUserHandler(service.Server(), NewUserHandler(log.WithField("service", QualifiedName), repo)); err != nil {
